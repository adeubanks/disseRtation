...
df %>% 
    ...
    as_gt() %>% 
    gt::fmt_markdown(columns = all_stat_cols()) %>% 
    gt::gtsave(file = "tables/demo_tbl_2016.html")
...
df %>% 
    ...
    as_gt() %>% 
    gt::fmt_markdown(columns = all_stat_cols()) %>% 
    gt::gtsave(file = "tables/demo_tbl_2020.html")
...
df %>% 
    ...
    as_gt() %>% 
    gt::fmt_markdown(columns = all_stat_cols()) %>% 
    gt::gtsave(file = "tables/studytbl2016.html")
...
df %>% 
    ...
    as_gt() %>% 
    gt::fmt_markdown(columns = all_stat_cols()) %>% 
    gt::gtsave(file = "tables/studytbl2020.html")
...
mod_plot_data %>% 
    #filter(model_type == "primary") %>% 
    #filter(str_detect(model_name, "mod_1")) %>% 
...
df %>% 
    ...
    as_gt() %>%
    gt::fmt_markdown(columns = all_stat_cols()) %>%
    gt::gtsave(file = "tables/missing_demo_tbl.html")
...
df %>% 
    ...
    as_gt() %>%
    gt::fmt_markdown(columns = all_stat_cols()) %>%
    gt::gtsave(file = "tables/missing_studytbl.html")
